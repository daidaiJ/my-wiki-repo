...
- `paths`：相对项目根的知识目录。**规约只认两个专用目录名：`wiki/` 与 `issues/`**（或整个目录即知识的平铺库声明 `"."`）；`docs/` 等与上游官方文档同名的通用目录**默认不接入**——避免把官方文档混进个人知识库，确需接入时由用户显式改声明。**规约要求：每个接入目录里必须有 `README.md` 索引该目录下的其他文档，由 agent 维护**（工具会在缺失时告警）
- **上游同名目录冲突**：clone 的上游项目若自带 `wiki/` 或 `issues/` 目录（上游自己的内容占用了专用名），看情况处理——内容对个人无价值则直接干掉让出名字；有价值则由 agent 整理提炼后合并进该项目个人知识库再删源目录，总之专用名必须归个人知识使用
