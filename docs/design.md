...
| 命令 | 时机 | 作用 |
|---|---|---|
| `wiki check` | 会话退出（唯一 hook） | 已注册项目：维护窗口、迁移正文、同步注册表；有 wiki-sync 声明块则自动接入；未注册且已有知识目录 → 自动接入（写注册表，后续会话按注册表维护）。行为策略跟随 `defaultMode`：`link`=反转（迁入+窗口链接），`copy`=简单拷贝（项目侧真目录保留，增量合并拷贝） |
